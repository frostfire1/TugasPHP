...
<body>
    <?php
    include "koneksi.php";
    $qry_get_siswa = mysqli_query($con, "select * from siswa where 
id_siswa = '" . $_GET['id_siswa'] . "'");
    $dt_siswa = mysqli_fetch_array($qry_get_siswa);9
    ?>
    <h3>Ubah Siswa</h3>
    <form action="proses_ubah_siswa.php" method="post">
        ...
        <select name="role" class="form-control">
            <?php
            if($dt_siswa['role'] == 'siswa'){$selekSiswa = "selected";}
            if($dt_siswa['role'] == 'admin'){$selekAdmin = "selected";}
            if($dt_siswa['role'] == 'super admin'){$selekSU = "selected";}
            echo '<option value="siswa" ' . $selekSiswa . '>Siswa</option>';
            echo '<option value="admin" ' . $selekAdmin . '>Admin</option>';
            echo '<option value="super admin" ' . $selekSU . '>Super Admin</option>';
            ?>
        </select>
        Kelas :
        <select name="id_kelas" class="form-control">
            <?php
            include "koneksi.php";
            $qry_kelas = mysqli_query($con, "select * from kelas");
